...
dp[i]={dp[i−1]+nums[i],ifdp[i−1]>0nums[i],ifdp[i−1]≤0 dp[i] = \begin{cases} dp[i - 1] + nums[i], & if \quad dp[i - 1] > 0 \\ nums[i], & if \quad dp[i - 1] \le 0 \end{cases}
dp[i]={ 
dp[i−1]+nums[i],
nums[i],
​
...
时间复杂度：O(N)O(N)O(N) ，这里 NNN 是输入数组的长度。
...
时间复杂度：O(Nlog⁡N)O(N \log N)O(NlogN)，这里递归的深度是对数级别的，每一层需要遍历一遍数组（或者数组的一半、四分之一）；
空间复杂度：O(log⁡N)O(\log N)O(logN)，需要常数个变量用于选取最大值，需要使用的空间取决于递归栈的深度。这里感谢 @bu-hui-codingde-xiao-cai-ji 朋友的提醒。
